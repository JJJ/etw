...
<?php global $basis_section, $basis_section_data, $basis_section_name, $basis_is_js_template; ?>
	...
	<div class="basis-section-sortable-stage basis-section-sortable-stage-profile">
		...
		<?php foreach ( array( 'left', 'middle', 'right' ) as $column ) : ?>
			<div class="basis-profile-column basis-sortable basis-profile-<?php echo $column; ?>-column" id="basis-profile-<?php echo $column; ?>-column">
				...
				<div class="basis-titlediv">
					<input placeholder="<?php esc_attr_e( 'Enter link here' ); ?>" type="text" name="<?php echo $basis_section_name; ?>[<?php echo $column; ?>][link]" class="basis-link widefat" value="<?php if ( isset( $basis_section_data[ $column ]['link'] ) ) echo esc_url( $basis_section_data[ $column ]['link'] ); ?>" autocomplete="off" />
				</div>
				...
				<div class="basis-titlediv">
					<div class="basis-titlewrap">
						<input placeholder="<?php esc_attr_e( 'Enter title here' ); ?>" type="text" name="<?php echo $basis_section_name; ?>[<?php echo $column; ?>][title]" class="basis-title basis-section-header-title-input" value="<?php if ( isset( $basis_section_data[ $column ]['title'] ) ) echo sanitize_text_field( $basis_section_data[ $column ]['title'] ); ?>" autocomplete="off" />
					</div>
				</div>
				...
				<?php else : ?>
					...
					<?php basis_get_html_builder()->wp_editor( $content, $basis_section_name . '[' . $column . '][content]', $mce_args ); ?>
				<?php endif; ?>
			</div>
		<?php endforeach; ?>
		...
		<input type="hidden" class="basis-section-order" name="<?php echo $basis_section_name; ?>[order]" value="" />
	</div>
