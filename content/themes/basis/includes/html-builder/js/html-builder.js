/* global BasisBuilderApp, jQuery, _, postboxes, pagenow, wpLink, wpActiveEditor, wpLinkL10n, tinyMCE, basisHTMLBuilderData, switchEditors */
(function (BasisBuilderApp, $, _) {
	...
	var BasisAdmin = function () {
		...
		function bindEvents() {
			...
			// Override the wpLink.open function
			cache.$document.on('ready', fixWPLinkOpen);
		}
		...
		/**
		 * Fix wpLink.open to account for brackets in editor name.
		 *
		 * wpLink.open takes the active editor name and uses it as a jQuery selector. When opening the link dialog for
		 * the builder editors, the ID will have brackets in it. In the sizzle selector engine, square brackets have
		 * special meaning and cause the function to break. This function is a duplicate of the core function; however,
		 * it escapes the brackets allowing the selector to function properly.
		 */
		function fixWPLinkOpen() {
			wpLink.open = function() {
				if (!wpActiveEditor) {
					return;
				}

				// Define this here as the var in the wpLink is private
				var inputs = {};
				inputs.dialog = $('#wp-link');

				/**
				 * Since many of the editor IDs will contain square brackets, escape them as they break the sizzle
				 * selector engine.
				 */
				var tempActiveEditor = wpActiveEditor.replace(/\[/g, '\\[').replace(/]/g, '\\]');
				wpLink.textarea = $('#'+tempActiveEditor).get(0);
