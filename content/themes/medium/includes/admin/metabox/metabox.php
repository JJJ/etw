 ...
 * @since 4.0
 */
function medium_add_video_box() {

	$screens = array( 'post', 'page' );
	foreach ( $screens as $screen ) {
		add_meta_box(
			'medium_video_section',
			__( 'Video', 'medium' ),
			'medium_inner_video_box',
			$screen,
			'normal',
			'high'
		);
...
add_action( 'add_meta_boxes', 'medium_add_video_box' );
 ...
 * @since 4.0
 */
function medium_inner_video_box( $post ) {

	// Use nonce for verification
	wp_nonce_field( plugin_basename( __FILE__ ), 'medium_video_box_nonce' );

	// Get existing value and use it for the value of the form
	$value = get_post_meta( $post->ID, 'video', true );
	$video_help = esc_url( 'https://array.is/articles/medium/?#creating-media-postsTB_iframe=true&amp;width=850&amp;height=600' );
	echo '</label> ';
	echo '<textarea rows="3" style="width:98%; margin-top: 10px;" id="medium_video_field" name="medium_video_field">'.esc_textarea($value).'</textarea>';
	echo '<p>';
		printf( __( 'Add video to your page by entering the video\'s <a class="thickbox" href="%1$s">embed code</a> in the box above (optional). ', 'medium' ), $video_help );
}
...
add_action( 'save_post', 'medium_save_video_meta' );