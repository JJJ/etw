...
final class MAKEPLUS_Admin_Notice extends MAKEPLUS_Util_Modules implements MAKEPLUS_Admin_NoticeInterface, MAKEPLUS_Util_HookInterface, MAKEPLUS_Util_LoadInterface {
	...
	public function hook() {
		...
		if ( is_admin() ) {
			...
			add_action( 'wp_ajax_makeplus_hide_notice', array( $this, 'handle_ajax' ) );
		}
	...
	public function load() {
		 ...
		 * @since 1.7.0.
		 *
		 * @param MAKEPLUS_Admin_Notice    $notice     The notice object that has just finished loading.
		 */
		do_action( 'makeplus_notice_loaded', $this );
	}
	...
	private function generate_one_time_admin_notice_id( WP_User $user = null ) {
		...
		return 'makeplus-notice-' . md5( $user->user_login );
	}
	...
	private function get_dismissed_notices( $user_id ) {
		$dismissed = get_user_meta( $user_id, 'makeplus-dismissed-notices', true );
		...
		if ( ! $dismissed ) {
			...
			if ( $dismissed ) {
				...
				delete_user_meta( $user_id, 'ttfmp-dismissed-notices' );
			}
		}
	...
	private function update_dismissed_notices( $user_id, array $notices ) {
		return update_user_meta( $user_id, 'makeplus-dismissed-notices', $notices );
	}
	...
	private function render_notices( $notices ) {
		...
		foreach ( $notices as $id => $args ) {
			...
			$nonce   = ( $dismiss && ! $onetime ) ? ' data-nonce="' . esc_attr( wp_create_nonce( 'makeplus_dismiss_' . $id ) ) . '"' : '';
			...
			// Render
			?>
			<div id="makeplus-notice-<?php echo esc_attr( $id ); ?>" class="<?php echo esc_attr( $classes ); ?>"<?php echo $nonce; ?>>
			...
			(function($) {
				$('.notice:not(.one-time)').on('click', '.notice-dismiss', function(evt) {
					...
					var $target = $(evt.target),
						...
						id      = $parent.attr('id').replace('makeplus-notice-', ''),
						nonce   = $parent.data('nonce');

					$.post(
						ajaxurl,
						{
							action : 'makeplus_hide_notice',
							nid    : id,
							nonce  : nonce
						}
					);
	...
	public function handle_ajax() {
		...
		if (
			...
			false === $nid ||
			false === $nonce ||
			! wp_verify_nonce( $nonce, 'makeplus_dismiss_' . $nid )
		) {
