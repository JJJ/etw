...
( function($) {
	...
	**/
	fusionPreview.updateContentBoxesPreview = function( thisRef, model, subElements ) {
		...
		innerHtml			= innerHtml.replace( $(innerHtml).find('font.content_boxes_columns').html() , subElements[5].value );
	...
	**/
	fusionPreview.updateImageFramePreview = function( thisRef, model, subElements ) {
		...
		innerHtml 			= innerHtml.replace( $(innerHtml).find('div.img_frame_section').html() , '<img src="'+subElements[8].value+'">' );
	...
	**/
	fusionPreview.updateImageCarouselPreview = function( thisRef, model, subElements ) {
		...
		var innerHtml 		= model.get('innerHtml');
		var totalElements 	= subElements[11].elements.length;
		var previewData		= '';

		for ( i = 0; i < totalElements; i ++) {
			element 	= subElements[11].elements[i];
			previewData+= '<li>';
			previewData+= ' <img src="'+element[2].value+'">';
			previewData+= '</li>';
	...
	**/
	fusionPreview.updateRecentPostsPreview = function( thisRef, model, subElements ) {
		...
		var cats 			= fusionParser.getUniqueElements( subElements[4].value ).join();
		cats 				= ( cats == '' ? 'All' : cats);
