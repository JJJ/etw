...
class FusionSC_RecentWorks {
	...
	function render( $args, $content = '') {
		...
		while ( $recent_works->have_posts() ) { 
			...
			// Reset vars
			$rich_snippets = $post_classes = $image = $post_title = $post_terms = $separator = $post_content = $buttons = $view_project_button = '';
			...
			if ( $layout == 'carousel' ) {
				...
				// Title below image layout
					...
					$image = avada_render_first_featured_image_markup( get_the_ID(), $this->image_size, get_permalink( get_the_ID() ), TRUE, FALSE, FALSE, 'disable', 'disable', '', $this->recent_works_counter );
					...
					// Get the terms
					$image .= get_the_term_list( get_the_ID(), 'portfolio_category', '<div class="fusion-carousel-meta">', ', ', '</div>' );					
				}				
				
				$portfolio_posts .= sprintf( '<li %s><div %s>%s%s</div></li>', FusionCore_Plugin::attributes( 'fusion-carousel-item'), FusionCore_Plugin::attributes( 'fusion-carousel-item-wrapper' ), 
											 avada_render_rich_snippets_for_pages(), $image );
					...
					if ( ! in_array( $portfolio_category->slug, $cats_to_exclude ) ) {
						...
						if ( ! empty( self::$args['cat_slug'] ) ) {
							...
							if ( in_array( $portfolio_category->slug, $cat_slugs ) ) {
								$filter .= sprintf( '<li %s><a %s>%s</a></li>', FusionCore_Plugin::attributes( 'fusion-filter fusion-hidden' ), 
