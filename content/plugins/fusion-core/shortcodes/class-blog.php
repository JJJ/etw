...
class FusionSC_Blog {
	...
	function render( $args, $content = '') {
		...
		if ( self::$args['load_more'] && self::$args['posts_per_page'] != -1 ) {
			$html .= sprintf( '<div class="fusion-load-more-button fusion-clearfix">%s</div>', __( 'Load More Posts', 'fusion-core' ) );
		}		
	...
	function loop_footer() {
		...
		if ( self::$args['meta_all'] && self::$args['layout'] == 'large' ||
			 self::$args['meta_all'] && self::$args['layout'] == 'medium'
		) {
